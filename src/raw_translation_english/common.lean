...
fail ("The name " ++ n.to_string ++ " is already being used.")
...
| fn _ [] := pure fn
| fn _ _ := fail "There are too many arguments."

/-- Apply an expression to a list of pre-expressions. -/
meta def mk_mapp_pexpr (fn : expr) (args : list pexpr) : tactic expr :=
do t ← infer_type fn >>= whnf,
   mk_mapp_pexpr_aux fn t args

/-- Apply a pre-expression to a list of pre-expressions. -/
meta def pexpr_mk_app : pexpr → list pexpr → pexpr
| e []      := e
| e (x::xs) := pexpr_mk_app (e x) xs

/-- Parse a list of arguments, possibly empty. -/
meta def apply_to_parse : lean.parser (list pexpr) :=
(tk "applying" *> tk "to" *> pexpr_list_or_texpr) <|> pure []
...
@[interactive]
meta def done_using (q : parse texpr) : tactic unit :=
do tgt : expr ← target,
...
/-- If `e` is a conjunction, we separet it and the list of two members, 
    otherwise we return [e]. -/
...
/- Separate all the conjugations in the list of expressions. -/
...
meta def conclude (pe : pexpr) (args : list pexpr) : tactic unit :=
focus1 (do
